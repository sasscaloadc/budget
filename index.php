...
//alert(item.prop("tagName")+" =0> "+ item.prop("id"));
			return eval(item.val().replace(/,/g, ''));
		} else {
			if (item.children(":first").prop("tagName") == "INPUT") {
//alert(item.prop("tagName")+" =1> "+ item.prop("id"));
				return eval(item.children(":first").val().replace(/,/g, ''));
			} else {
				if ((typeof item.html() == 'undefined') || (isNaN(eval(item.html().replace(/,/g, ''))))) {
					return 0;
				} else {
//alert(item.prop("tagName")+" => "+ item.prop("id"));
					return eval(item.html().replace(/,/g, ''));
				}
			}
		}
	}
...
//alert("after");
	...
	function update_value(item) {
		...
		value1 = val(item);
		value2 = ext == "_euro" ? value1 / val($("#localxrate")) : value1 * val($("#localxrate"));
		vset($("#"+base+ext), roundToTwo(value2)); 
		vset($("#"+base+"_available"), roundToTwo(task[base+"_budget"] - task["cum_"+base+"_euro"] - val($("#"+base+"_euro"))));
				//$("#transport_available").html(roundToTwo(task.transport_budget - task.cum_transport_euro - val($("#transport_euro"))));
		update_totals();
	}
	...
	function load_figures() {
		...
		$.get("load_figures_data.php?database=budget&taskid="+ $("#tasks").val().trim()+"&year=" + $("#years").val().trim() + "&quarter=" + $("#quarters").val().trim(), function(data, status){
				...
				$("#investments_available").html(roundToTwo(task.investments_budget - task.cum_investments_euro - val($("#investments_euro"))));
				$("#services_available").html(roundToTwo(task.services_budget - task.cum_services_euro - val($("#services_euro"))));
				$("#consumables_available").html(roundToTwo(task.consumables_budget - task.cum_consumables_euro - val($("#consumables_euro"))));
				$("#transport_available").html(roundToTwo(task.transport_budget - task.cum_transport_euro - val($("#transport_euro"))));
				$("#personnel_available").html(roundToTwo(task.personnel_budget - task.cum_personnel_euro - val($("#personnel_euro"))));
				update_totals();
		});
	}
